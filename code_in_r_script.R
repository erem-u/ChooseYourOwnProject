...
# TITLE OF THE PROJECT:  PREDICTION OF FAILURE ON ROLLER BEARINGS USING VIBRATION ANALYSIS
...
# This project is specific to mechanical engineering. I am a mechanical engineer
# and have always wanted to learn this technique so have chosen this subject for the project to add my professional skillset.
# Readers/reviewers who do not have engineering background might struggle to understand the concept of vibration analysis.
# I will try to explain every step clearly and provide satisfactory reasoning for each step.
# The purpose of the project is to imitate the role of a vibration analysis expert
# who has the skills and experience to assess vibration data which is collected with sensors
# and make reasonable predictions whether the bearings of the interest would fail or not in certain time period.
# Roller bearings are the most problematic component of rotating equipment
# to which special maintenance attention are given to prevent dramatic consequences. 
# Three types of prediction algorithms have been used in this project. Those are Random Forest, KNN and Stochastic Gradient Boosting.
# These three algorithms are compared in terms of overall accuracy in the Results section.
# It turns out that Random Forest model provides the highest accuracy.
...
#   Section 1.1 - What is Vibration Anaylsis?
#
# Section 2 - ANALYSIS
#   Section 2.1 - Data Wrangling
#   Section 2.2 - Analyzing the Vibration Data
#       Section 2.2.1 - Time-Domain Anaylsis
...
# The data was collected in a run-to-failure experiment performed in 2006. It consists four roller bearings on a loaded shaft
# that rotates at a constant speed. Three datasets were generated in the experiment.
# In set 1, two high precision accelerometers were installed on each bearing and measurements were taken with all 8 accelerometers.
# whereas one accelerometer was used in datasets 2 and 3.
# Each dataset is formatted in individual files, each containing a 1-second vibration signal snapshot that was recorded every other 10 minutes.
# Each file consists of 20,480 points with a sampling rate set of 20 kHz.
...
# The dataset is loaded to Google Drive since it is too large to be unloaded to Github.
# You will need to save it from Google Drive to your computer if you want to run this code. Here is the link for the dataset:
# https://drive.google.com/drive/u/0/folders/12Xc5rP9pjmphU3cZ-v_OoQA5wFAXHPLK
...
# Vibration analysis is defined as a process for measuring the vibration levels and frequencies of machinery or a component
# and analyzing how healthy the machines and their components are.
# It all starts with using an accelerometer to measure vibration.
# Anytime a piece of machinery is running, it vibrates.
# An accelerometer attached to the machine generates a voltage signal that corresponds to the amount and frequency of vibration the machine
# usually how many times per second or minute the vibration occurs.

# All data collected from the accelerometer goes directly into a data collector (software) 
# which records the signal as either amplitude vs. time (known as time waveform),
# amplitude vs. frequency (known as Fast Fourier Transfor spectrum) or both.
# All of this data is analyzed by computer program algorithms, which in turn is analyzed by engineers or trained vibration analysts
# to determine the health of the machine and identify possible impending problems
# like looseness, unbalance, misalignment, lubrication issues and more.
...
# meaning that we have prior knowledge of what the output values for our samples should be.
# A vibration analysis expert has reviewed the data and
# drawn the conclusions below about the failure patterns for the bearings included in the experiment:
...
# Normal operation of the bearing occured between these time frame: 2013.1023.09.24.13 - 2003.11.19.21.06.07
...
# The purpose of this project is to develop a machine learning algorihtm
# that will imitate the function of a vibration analysis expert to predict bearing failure before it happens.
...
# The folder "1st_test" that contains the vibration analysis data is stored in Google Drive.
# There are 2156 files in this folder. Each file contains 20480 lines of vibration measurement.
# Let us took at the titles of first a few files:
files <- list.files("./1st_test")
head(files)


# These files have been titled in this format: YYYY.MM.DD.HR.MM.SS
# Let us have a look at the first file:
file <- read.table(paste("./1st_test/", files[1], sep=""), 
                   col.names = paste("b", rep(1:4, each = 2), c("x", "y"), sep = ""),
                   sep = "\t")
head(file)
# As you can see in the dataframe, there are two sets of vibration reading for each bearing (4 bearing total x 2 readings for each)
# x and y represent the locations of the accelerometers on a bearing.
# One accelerometer measures vibration occuring vertically, the other one horizontally.


# Now let us plot the values taken by the each accelerometer versus time in the first file.
# The variable "file" is matched to the first file.
# There will be 8 different graphs total:
graph_1 <- file %>%
...
graph_1
# You will notice that the mean vibration value on each graph is negative.
# This is due to the accelerometers having negative voltage when measuring and negative values do not mean problem.  
# Furthermore, the y-axis vibration on bearing 1 (second figure from the top left corner) seems to have outliers
# but they do appear at regular-ish intervals.
...
#######################################################################################################################################
####  IMPORTANT NOTE:   In Secton 2.2, the raw vibration data (measurements taken by accelerometers) is processed to obtain a dataset
####                    which will be used in prediction models. Therefore, the reader/reviewer is free to skip the Section 2.2 and
####                    continue on Section 2.3
#######################################################################################################################################  
  
  
# Working with the raw vibration data is not the best approach for failure prediction.
# Therefore, some certain analysis techniques are needed for further anaylsis. These are:
  
# Time-Domain Anaylsis (Peak, Average, Peak-to-Peak, RMS, Skewness, Kurtosis, Entropy)
...
# This function will read the file and tabulates the vibration readings in easy-to-read format:
...
# This function calculates the Peak-to-Peak (positive peak + negative peak) value in a sinusoidal signal:
p2p <- function(x){abs(max(x)) + abs(min(x))} 

# This function calculates the Shannon entropy (cut signal to 500 bins) of a sinusoidal signal:
...
# Now let us combine all the functions defined previously into a single function for Time-Domain Analysis a sinusoidal signal:
...
# A method of viewing the vibration signal in a way that is more useful for analysis is to apply a Fast Fourier Transformation (FFT).
# In non-mathematical terms, this means that the signal is broken down into specific amplitudes at various component frequencies.
# An FFT spectrum is an incredibly useful tool.
# If a machinery problem exists, FFT spectra provide information to help determine the location of the problem,
# the cause of the problem and with trending,
# how long until the problem becomes critical.
# Because we know that certain machinery problems occur at certain frequencies,
# we analyze the FFT spectrum by looking for amplitude changes in certain frequency ranges.
...
graph_2 <- read_file(files[2000])$b2y %>%
  apply_FFT(.) %>%
  ggplot(aes(x = freq, y = log(amp))) + 
  geom_line(colour = "blue", alpha = 0.5) + 
  ggtitle("FTT Specturm")
graph_2

# Looks about right (qualitatively), noisy but more or less as expected. R has a base function to
# approximate the spectral density of a stationary signal, by fitting an autoregressive model on it. This might be
# helpful as the expected result will be much less noisy. Let us try it out:
...
spec <- spec.ar(x = raw_vib, 
                ...
                order = 20, # This parameter is to change smoothness of the density
                plot = F) # Do not plot
spec <- data.frame("freq" = seq(0, 20000 / 2, length.out = length(spec$spec)),
                   "amp" = log(spec$spec))

graph_3 <- ggplot(spec, aes(x = freq, y = amp)) + 
  geom_line(colour = "blue", alpha = 0.5) + 
  ggtitle("Spectral Density Chart")
graph_3
# That is a nice result. The peaks are clearly defined and the result is less noisy overall. We will be using this
# function for the rest of the project. Before we move any further, we should calculate the characteristic frequencies
...
# A failure in one of the bearing components will produce a pronounced peak in a characteristic frequency of the frequency spectrum.
# This characteristic frequency will allow quick and easy identification of the failure.
...
# The FTF defect will always occur at about 40 percent of running speed.
# Of the remaining three bearing defect frequencies BSF is always the lowest frequency.
# The ball pass–inner race frequency is always the highest.
...
# We will also need to assign the labels to the dataset.
# We will use the failure patters that are provided in Section 2 as labels:
...
# And finally, let us write a small function to perfrom a bit of post-processing on the dataset, to bring it into a format
...
# All done! Let us extract the features for the entire dataset and store them in a .csv file. Note that we do not
# necessairly need the filenames themselves as the dataset is already chronologically ordered due to the filename format
...
# Let’s first assess predictor importance. We’ll be using a model-based approach, based on a random forest
# classifier. The reason for choosing a model-based approach is that, being tied to model performance, it may be
# able to incorporate the correlation structure between the predictors into the importance calculation.
...
graph_4 <- plot(rfImp, top = 10)
graph_4

ggplot(rfImp, aes(x = freq, y = amp)) + 
  geom_line(colour = "blue", alpha = 0.5) + 
  ggtitle("Spectral Density Chart")
graph_3

# We can see that each predictor has different importance for each of the classes (reading the documentation of varImp,
# that is to be expected from tree-based algorithms). It is also interesting to note that vibration power levels
# at characteristic frequencies are not in the top 10 predictors.
...
graph_5 <- train[, top_predictors] %>%
  cor(., method = "spearman") %>%
  ggcorrplot::ggcorrplot(., hc.order = TRUE, type = "lower", lab = T, lab_size = 2.7, show.diag = F) +
  ggtitle("Spearman Correlations")
graph_5

# Highest correlation coefficient is 0.92. We have moderately correlated data to this point. Note that these are
# monotonic relations and not necessarily linear. 



# Let’s make a boxplot to visualize the underlying distributions:

graph_6 <- train[, top_predictors] %>%
...
# There are noticeable differences between groups for variables x_entropy, x.ar3 (inner race failure and stage 2 failure),
# x.ar6, y.ar1, y.ar4, x.ar5. Let’s isolate these predictors and make a pair plot:
...
graph_7
# Indeed, some clusters have started to emerge but nothing easily separable. Let us begin modeling and depending
# on the results, we might return to more advanced feature selection methods
...
# Three different algorithms were selected for predictive modeling to choose the one with the best overall accurarcy. 
# These algorithms are Random Forest, KNN and Stochastic Gradient Boosting. We will start with KNN.
...
# Firstly, I will split the train dataset that was created earlier to another train and test datasets to train my candidate algorithims:
set.seed(2)
...
# We obtain Overall Accuracy of 0.782 and Kappa of 0.654. Prevalence among the classes are within reasonable levels. 
# Accuracy is pretty low with KNN model.
...
# We obtain Overall Accuracy of 0.9242 and Kappa of 0.8819. Prevalence among the classes are within reasonable levels.
# Accuracy is better than what was obtained with KNN.
...
# We obtain Overall Accuracy of 0.9391 and Kappa of 0.9065. Besides, Prevalence among the classes are within reasonable levels.
# It is highest accuract obtained among these three models.
...
confusionMatrix(y_hat_rf_fnl, as.factor(validation$state))

# We obtain Overall Accuracy of 0.933 and Kappa of 0.8964. Prevalence among the classes are within reasonable levels.

# Indeed, we get similar results on the prediction set as before. Small confusion on the suspect class, very little to no
# confustion between early and normal health state, as well as the different failure modes.
...
# The results that were obtained by each model are presented and discussed in this section. Firstly, let us tabulate the results
# in the table below:

#  Method                           Accuracy
# KNN                                0.782
# Stochastic Gradient Boosting       0.924
# Random Forest                      0.939
# Final Model (Random Forest)        0.933


# As seen the results in the table above, the highest accuracy was obtained with the Random Forest model.
# Therefore, Random Forest was selected to compute the Final Accuracy with the Validation Set in Section 2.7.
...
# We have built a classifier that can determine the health status of roller bearings
# and experimented quite a lot with feature extraction (and reduction)
# which led us to choose 8 features from the two vibration signals (x- and y- axis).
# We have managed to get a 93% accuracy and 90% Kappa value on the validation set
# but the errors are to be expected: There is small level of confusion between early and normal data
# as well as between suspect and the different failure modes. It is also nice to see that there is very little
# confusion between the classes relating to good health and those of bad health. In addition, the failure classes are
# only ever classified as different types of failures and never as normal behaviour. The most confusion seems to be
# in the suspect class but that is understandable, considering that the suspect class is a just a transition from
# normal to a failure pattern.

# As explained earlier, this is a classifier model. However, a regression approach could also be followed to predict
# the Remaining Useful Life (RUL) of the bearings. Another model that satisfies the RUL approach could be applied as further analysis
# if the goal is to understand how much time is left before failure occurs.
